...
export const cancelCommand = buildCommand(async ({ readExecutor, tgUser, publicHttpApi, send, log }, _, { runCommand }) => {
  try {
    log('CANCEL')
    ...
    if (session) {
      if (session.state === 'creatingProductsRequest' || session.state === 'creatingChangeCityRequest') {
      ...
      if (session.state === 'choosingCity' || session.state === 'confirmingCity') {
        if (session.activeChangeCityRequestId) {
          return publicHttpApi.changeCityRequest.POST.cancel({
